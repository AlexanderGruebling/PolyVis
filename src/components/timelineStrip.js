...
export async function createTimelineStrip() {
    ...
    await q(`CREATE OR REPLACE TABLE tl_arou AS SELECT "Sample#"::DOUBLE AS x FROM arou`);
    await q(`CREATE OR REPLACE TABLE tl_hyp  AS SELECT "Sample#"::DOUBLE AS x FROM resp WHERE Aux = 'Hyp'`);
    await q(`CREATE OR REPLACE TABLE tl_ob   AS SELECT "Sample#"::DOUBLE AS x FROM resp WHERE Aux = 'Ob.A'`);
    await q(`CREATE OR REPLACE TABLE tl_cn   AS SELECT "Sample#"::DOUBLE AS x FROM resp WHERE Aux = 'Cn.A'`);
    ...
    if (desatIntervals.length > 0) {
        ...
        await q(`CREATE OR REPLACE TABLE tl_desats AS SELECT * FROM (VALUES ${rows}) AS t(x)`);
    } else {
        await q(`CREATE OR REPLACE TABLE tl_desats AS SELECT 0::DOUBLE AS x WHERE FALSE`);
    }
    ...
    TRACKS.forEach((track, i) => {
        ...
        const plotMarks = [
            ...marks,
            vg.ruleX({ x: params.hypnPoint, stroke: "#ffffff", strokeOpacity: 0.5, strokeWidth: 1 }),
            vg.xDomain(domain),
            vg.yDomain([0, 1]),
            vg.height(14),
            vg.marginTop(1),
        ...
        if (isLast(i)) {
            ...
            plotMarks.push(vg.xTickFormat(() => ""), vg.xLabel(null));
        }
